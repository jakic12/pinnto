...
class Load(object):
    ...
    # compute 2D index from 1D position for node (boundary of element)
    def elpos(self, ntopleft):
        """
        Calculates the x and y position of the requested topleft node

        Parameters
        ---------
        ntopleft : int
            The node number of the requested node.
...
class PullOut(Load):
    ...
    def force(self):
        """
        The force vector containts a load on all corners pulling outwards.
        """
        f = super().force()
        n1, _, _, _ = self.nodes(0, 0)
        _, n2, _, _ = self.nodes(self.nelx-1, 0)
        _, _, n3, _ = self.nodes(self.nelx-1, self.nely-1)
        _, _, _, n4 = self.nodes(0, self.nely-1)
    ...
    def fixdofs(self):
        ...
        return [x * self.dim for x in self.nodes(self.nelx//2, self.nely//2)]
...
class BiAxial(Load):
    ...
    def passive(self):
        ...
        return elx, ely, values, list(range(self.nelx*self.nely))
